...
function chartPoints() {
  const range = byId('trendRange').value; const points = [...records].sort((a, b) => a.date.localeCompare(b.date)); const filtered = range === 'all' ? points : points.filter((point) => { const start = range === 'month' ? formatDate(calendarMonth) : formatDate(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() - 2, 1)); const end = formatDate(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() + 1, 0)); return point.date >= start && point.date <= end; });
  const dates = new Map(); filtered.forEach((point) => { const day = dates.get(point.date) || { date: point.date }; day[point.period] = point.weight; dates.set(point.date, day); }); return [...dates.values()];
}
function drawChart() {
  const canvas = byId('trendChart'); const empty = byId('emptyChart'); const detail = byId('chartDetail'); const points = chartPoints(); empty.classList.toggle('hidden', points.length > 0); detail.classList.toggle('hidden', !points.length); const context = canvas.getContext('2d'); const rect = canvas.getBoundingClientRect(); const ratio = window.devicePixelRatio || 1; canvas.width = rect.width * ratio; canvas.height = rect.height * ratio; context.scale(ratio, ratio); context.clearRect(0, 0, rect.width, rect.height); if (!points.length) return;
  if (chartSelection > points.length - 1) chartSelection = points.length - 1; const selected = chartSelection; const selectedPoint = points[selected]; detail.textContent = `${selectedPoint.date.replaceAll('-', '.')}\n早 ${selectedPoint.morning?.toFixed(2) ?? '—'} · 晚 ${selectedPoint.evening?.toFixed(2) ?? '—'} kg`;
  const width = rect.width; const height = rect.height; const padding = { top: 24, right: 20, bottom: 38, left: 48 }; const weights = points.flatMap((point) => [point.morning, point.evening]).filter(Number.isFinite); let min = Math.floor((Math.min(...weights) - 1) * 2) / 2; let max = Math.ceil((Math.max(...weights) + 1) * 2) / 2; if (min === max) { min -= 1; max += 1; }
  ...
  [['morning', '#2f7555'], ['evening', '#5476b8']].forEach(([period, color]) => { context.strokeStyle = color; context.lineWidth = 3; context.beginPath(); let started = false; points.forEach((point, index) => { if (!Number.isFinite(point[period])) { started = false; return; } if (started) context.lineTo(x(index), y(point[period])); else { context.moveTo(x(index), y(point[period])); started = true; } }); context.stroke(); points.forEach((point, index) => { if (!Number.isFinite(point[period])) return; context.fillStyle = color; context.beginPath(); context.arc(x(index), y(point[period]), 4, 0, Math.PI * 2); context.fill(); }); });
  context.strokeStyle = '#2f5a43'; context.lineWidth = 1; context.setLineDash([4, 4]); context.beginPath(); context.moveTo(x(selected), padding.top); context.lineTo(x(selected), height - padding.bottom); context.stroke(); context.setLineDash([]); detail.style.left = `${Math.min(Math.max(x(selected) + 8, padding.left + 4), width - 160)}px`; detail.style.top = `${padding.top + 4}px`; context.fillStyle = '#65746a'; context.textAlign = 'center'; const labelCount = Math.min(6, points.length); const step = Math.max(1, Math.ceil((points.length - 1) / Math.max(1, labelCount - 1))); points.forEach((point, index) => { if (index % step === 0 || index === points.length - 1) context.fillText(point.date.slice(5).replace('-', '/'), x(index), height - 12); }); context.textAlign = 'start';
}
function moveChartLine(event) { const points = chartPoints(); if (!points.length) return; const rect = byId('trendChart').getBoundingClientRect(); const padding = { left: 48, right: 20 }; const width = rect.width - padding.left - padding.right; chartSelection = points.length === 1 ? 0 : Math.round(Math.max(0, Math.min(width, event.clientX - rect.left - padding.left)) / width * (points.length - 1)); drawChart(); }
...
byId('previousMonth').addEventListener('click', () => { calendarMonth = new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() - 1, 1); renderCalendar(); drawChart(); }); byId('nextMonth').addEventListener('click', () => { calendarMonth = new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() + 1, 1); renderCalendar(); drawChart(); }); byId('trendRange').addEventListener('change', () => { chartSelection = 0; drawChart(); }); byId('trendChart').addEventListener('pointerdown', (event) => { event.currentTarget.setPointerCapture(event.pointerId); moveChartLine(event); }); byId('trendChart').addEventListener('pointermove', (event) => { if (event.buttons) moveChartLine(event); });
